...
def get_os_data_path() -> str:
    ...
    if user_os == const.LINUX:
        return os.environ.get(
            const.ENV_XDG_DATA_HOME, os.path.expanduser(const.PATH_LINUX_DATA)
        )
    elif user_os == const.MACOS:
        return os.path.expanduser(const.PATH_MACOS_DATA)
    elif user_os == const.WINDOWS:
        return os.environ.get(
            const.ENV_LOCAL_APPDATA, os.path.expanduser(const.PATH_WINDOWS_DATA)
        )
    else:
...
def initialize_user_data() -> None:
    app_data_dir_path: str = os.path.join(get_os_data_path(), const.APP_DIR_NAME)
